...
void CAN_Filter_Mask_Config(CAN_HandleTypeDef* hcan, uint8_t para, uint32_t Id, uint32_t Mask) {
    ...
    uint8_t rtr_type = para & 0x01;
    uint8_t id_type = (para >> 1) & 0x01;
    uint8_t fifo_id = (para >> 2) & 0x01;
    uint8_t filter_id = (para >> 3);

    if (id_type) {
        /* ExtId */
        canFliter.FilterIdHigh = (uint16_t)(Id >> 13);
        canFliter.FilterIdLow = (uint16_t)((Id << 3) | (id_type << 2) | (rtr_type << 1));

        canFliter.FilterMaskIdHigh = (uint16_t)(Mask >> 13);
        canFliter.FilterMaskIdLow = (uint16_t)(Mask << 3) | (1 << 2) | (1 << 1);
    } else {
        /* StdId */
        canFliter.FilterIdHigh = (uint16_t)(Id << 5);
        canFliter.FilterIdLow = (uint16_t)(id_type << 2) | (rtr_type << 1);

        canFliter.FilterMaskIdHigh = (uint16_t)(Mask << 5);
        canFliter.FilterMaskIdLow = (uint16_t)(1 << 2) | (1 << 1);
    }
    canFliter.FilterFIFOAssignment = (fifo_id) ? CAN_FILTER_FIFO1 : CAN_FILTER_FIFO0;
    canFliter.FilterBank = filter_id;
