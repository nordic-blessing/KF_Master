...
void uart_IT_protocol_init() {
    HAL_UART_Receive_IT(&huart4, &KFS_CheatHandler.receive_byte, 1);
    HAL_UART_Receive_IT(&huart5, &visual_uart.receive_byte, 1);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    ...
    uart_RX_decode(huart, &acquisition_uart_FM);
    #endif
        ...
        uart_RX_decode(huart, &wit_uart);
    #endif
    */
    if (huart->Instance == UART4) {
        uart_RX_decode(huart, &KFS_CheatHandler);
